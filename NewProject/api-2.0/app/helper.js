...
const getCCP = async(org) => {
    ...
    if (org == "Org1") {
        ...
        // } else if (org == "Org2") { //We can register in other orgs but we only want Dean of Students to be the registration authority
...
const getRegisteredUser = async(username, name, userOrg, studentStanding, studentSchool, isJson) => {
    let ccp = await getCCP(userOrg)
    if (ccp == null) {
        var response = {
            success: false,
            message: 'Invalid Organization. Only the Dean of Students can register students',
        }
    }
    ...
    if (userIdentity) {
        ...
        var response = {
            success: true,
            message: username + ' has already been enrolled Successfully',
        };
        return response
    }
    ...
    if (studentSchool != "School of Arts and Science" && studentSchool != "School of Business" && studentSchool != "School of Architecture and Design") {
        var response = {
            success: false,
    ...
    console.log(`Successfully registered and enrolled by admin the user ${username} and imported it into the wallet`);


    // Perform the transaction invocation
    const channelName = 'mychannel'; //Restrict by hardcoding the vars
    const chaincodeName = 'evote';
    const fcn = 'RegisterUser';
    const args = [username, name, studentSchool, studentStanding];
    ...
    var response = {
        success: true,
        message: `${username} enrolled successfully on the network and the transaction ${fcn} was invoked`,
